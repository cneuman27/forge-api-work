...
import { C3DRenderOptions_HollowModel } from "./C3DRenderOptions_HollowModel";
...
export class CMOM3DGenerator_HollowModel
{
    private _options: C3DRenderOptions_HollowModel = null;
    private _facePolygonDataList: CFacePolygonData[] = [];

    public generateModel(options: C3DRenderOptions_HollowModel): C3DModel
    {
        ...
        if (this._options.geometry.xlength <= this._options.wallThickness_Front + this._options.wallThickness_Rear ||
            this._options.geometry.ylength <= this._options.wallThickness_Bottom + this._options.wallThickness_Top ||
            this._options.geometry.zlength <= this._options.wallThickness_Left + this._options.wallThickness_Right)
        {
            return null;
        }
        ...
        model.facesRenderedSeparately = this._options.renderFacesSeparately;
        ...
        for (let vectorData of CConstants.FACE_VECTOR_DATA)
        {
            ...
            geometryCollection = this.renderFace(vectorData.face);

            if (this._options.renderFacesSeparately)
            {
                // #region Exterior Meshes

                if (geometryCollection.exteriors.length > 0)
                {
                    geometry = CUtility.combineGeometries(geometryCollection.exteriors);
                    CUtility.generatePlanarTextureCoordinates(geometry, vectorData.face);

                    mesh = new THREE.Mesh(geometry, this._options.material_Exterior);
                    model.exteriorModelListMap.get(vectorData.face).push(mesh);
                }
                ...
                if (geometryCollection.interiors.length > 0)
                {
                    geometry = CUtility.combineGeometries(geometryCollection.interiors);
                    CUtility.generatePlanarTextureCoordinates(geometry, vectorData.face);

                    mesh = new THREE.Mesh(geometry, this._options.material_Interior);

                    model.interiorModelListMap.get(vectorData.face).push(mesh);
                }
                ...
                if (geometryCollection.edges.length > 0)
                {
                    geometry = CUtility.combineGeometries(geometryCollection.edges);
                    CUtility.generatePlanarTextureCoordinates(geometry, vectorData.face);

                    mesh = new THREE.Mesh(geometry, this._options.material_Edge);

                    model.edgeModelListMap.get(vectorData.face).push(mesh);
                }
        ...
        if (this._options.renderFacesSeparately == false)
        {
            // #region Exterior Meshes
    ...
    private renderFace(face: Enums.E_Location): CGeometryCollection
    {
        ...
        for (let polygon of facePolygonData.exteriorPolygonList)
        {
                ...
                for (let x = 0; x < polygon.polygon_Poly2Tri.length; x++)
                {
                    ...
                    if (face === Enums.E_Location.Front)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            this._options.geometry.x + this._options.wallThickness_Front,
                            lastPoint.y,
                            lastPoint.x));
                    ...
                    else if (face === Enums.E_Location.Rear)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            this._options.geometry.xend - this._options.wallThickness_Rear,
                            lastPoint.y,
                            lastPoint.x));
                    ...
                    else if (face === Enums.E_Location.Bottom)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            lastPoint.x,
                            this._options.geometry.y + this._options.wallThickness_Bottom,
                            lastPoint.y));
                    ...
                    else if (face === Enums.E_Location.Top)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            lastPoint.x,
                            this._options.geometry.yend - this._options.wallThickness_Top,
                            lastPoint.y));
                    ...
                    else if (face === Enums.E_Location.Left)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            lastPoint.x,
                            lastPoint.y,
                            this._options.geometry.z + this._options.wallThickness_Left));
                    ...
                    else if (face === Enums.E_Location.Right)
                    {
                        ...
                        geometry.vertices.push(new THREE.Vector3(
                            lastPoint.x,
                            lastPoint.y,
                            this._options.geometry.zend - this._options.wallThickness_Right));
            ...
            if (renderBorder)
            {
                // #region Render Opening Profiles - Only Applies to Exterior Side
        ...
        // #region Interior Polygons

        for (let polygon of facePolygonData.interiorPolygonList)
        {
            perimeter_main = [];
            ...
            if (renderBorder)
            {
                ...
                geometry = CUtility.generateGeometryFromTriangles(
                    ...
                    face,
                    this._options.geometry,
                    this._options.getWallThickness(face),
                    true);
        ...
        spannedOpenings: C3DOpening[] = null): CFacePolygonData
    {
        ...
        if (this._options.getWallThickness(face) === 0)
        {
            // #region Open - Add Single Opening (Perimeter Less Wall Thicknesses)

            opening = new C3DOpening();

            // #region Set Geometry Based on Face
                ...
                face === Enums.E_Location.Rear)
            {
                ...
                opening.geometry.y = this._options.geometry.y + this._options.wallThickness_Bottom;
                opening.geometry.z = this._options.geometry.z + this._options.wallThickness_Left;

                opening.geometry.xlength = 0;
                opening.geometry.ylength = this._options.geometry.ylength - (this._options.wallThickness_Top + this._options.wallThickness_Bottom);
                opening.geometry.zlength = this._options.geometry.zlength - (this._options.wallThickness_Left + this._options.wallThickness_Right);
                ...
                face === Enums.E_Location.Top)
            {
                ...
                opening.geometry.x = this._options.geometry.x + this._options.wallThickness_Front;
                opening.geometry.y = this._options.geometry.y;
                opening.geometry.z = this._options.geometry.z + this._options.wallThickness_Left;

                opening.geometry.xlength = this._options.geometry.xlength - (this._options.wallThickness_Front + this._options.wallThickness_Rear);
        ...
        {
            // #region Closed - Retrieve All Openings on Face

            openingList = _.filter(
                this._options.openingList,
                function (i: C3DOpening): boolean
                {
                    return i.face === face &&
                           i.isValid === true;
                });
        ...
        container3DRect = new CRect3D(
            this._options.geometry.x + this._options.wallThickness_Front,
            this._options.geometry.y + this._options.wallThickness_Bottom,
            this._options.geometry.z + this._options.wallThickness_Left,
            this._options.geometry.xlength - (this._options.wallThickness_Front + this._options.wallThickness_Rear),
            this._options.geometry.ylength - (this._options.wallThickness_Bottom + this._options.wallThickness_Top),
            this._options.geometry.zlength - (this._options.wallThickness_Left + this._options.wallThickness_Right));

        data.interiorRect = CUtility.get2DRectForFace(
            container3DRect,
            face);
        ...
        facePath.push(new ClipperLib.IntPoint(data.interiorRect.x, data.interiorRect.y));
        facePath.push(new ClipperLib.IntPoint(data.interiorRect.xend, data.interiorRect.y));
        facePath.push(new ClipperLib.IntPoint(data.interiorRect.xend, data.interiorRect.yend));
        facePath.push(new ClipperLib.IntPoint(data.interiorRect.x, data.interiorRect.yend));
        ...
        for (let f of solution.Childs())
        {
            ...
            data.interiorPolygonList.push(polygonData_face);
        }
        ...
        if (data.face === Enums.E_Location.Front)
        {
            ...
            for (let line of sideData.edgeList_Right)
            {
                if (line.startPoint.y != lastEnd)
                {
                    ...
                    opening.geometry.x = this._options.geometry.x;
                    opening.geometry.xlength = 0;
                    opening.geometry.y = data.exteriorRect.yend - this._options.wallThickness_Top;
                    opening.geometry.ylength = this._options.wallThickness_Top;
            ...
            for (let line of sideData.edgeList_Right)
            {
                if (line.startPoint.y != lastEnd)
                {
                    ...
                    opening.geometry.ylength = this._options.wallThickness_Bottom;
                    opening.geometry.z = lastEnd;
                    opening.geometry.zlength = line.startPoint.y - lastEnd;
            ...
            for (let line of sideData.edgeList_Right)
            {
                if (line.startPoint.y != lastEnd)
                {
                    ...
                    opening.geometry.zlength = this._options.wallThickness_Left;
            ...
            for (let line of sideData.edgeList_Left)
            {
                if (line.startPoint.y != lastEnd)
                {
                    ...
                    opening.geometry.y = lastEnd;
                    opening.geometry.ylength = line.startPoint.y - lastEnd;
                    opening.geometry.z = data.exteriorRect.xend - this._options.wallThickness_Right;
                    opening.geometry.zlength = this._options.wallThickness_Right;
        ...
        else if (data.face === Enums.E_Location.Top)
        {
            ...
            for (let line of sideData.edgeList_Top)
            {
                if (line.startPoint.x != lastEnd)
                {
                    ...
                    opening.geometry.xlength = this._options.wallThickness_Front;
            ...
            for (let line of sideData.edgeList_Top)
            {
                if (line.startPoint.x != lastEnd)
                {
                    ...
                    opening.geometry.x = data.exteriorRect.xend - this._options.wallThickness_Rear;
                    opening.geometry.xlength = this._options.wallThickness_Rear;
                    opening.geometry.y = this._options.geometry.yend;
                    opening.geometry.ylength = 0;
                    opening.geometry.z = lastEnd;
                    opening.geometry.zlength = line.startPoint.x - lastEnd;
            ...
            for (let line of sideData.edgeList_Top)
            {
                if (line.startPoint.x != lastEnd)
                {
                    ...
                    opening.geometry.y = this._options.geometry.yend;
                    opening.geometry.ylength = 0;
                    opening.geometry.z = data.exteriorRect.yend - this._options.wallThickness_Right;
                    opening.geometry.zlength = this._options.wallThickness_Right;
        ...
        else if (data.face === Enums.E_Location.Left)
        {
            ...
            for (let line of sideData.edgeList_Left)
            {
                if (line.startPoint.y != lastEnd)
                {
                    ...
                    opening.geometry.xlength = this._options.wallThickness_Front;
                    opening.geometry.y = lastEnd;
                    opening.geometry.ylength = this._options.wallThickness_Front;
                    opening.geometry.z = this._options.geometry.z;
                    opening.geometry.zlength = 0;
        ...
        face: Enums.E_Location)
    {
        ...
        for (let x = 0; x < profile.vertexCount; x++)
        {
            ...
            // #region Render Exterior Profile (on Exterior Face) Based On Location
            ...
            // #region Render Interior Profile (on Interior Face) Based on Location
